...
int main() {
    ...
    for(int i=0; i<tmp.size(); i++){
        ...
        if((tmp[i] <'0' || tmp[i] >'9') && tmp[i]!='+' && tmp[i]!='-' && tmp[i]!='/' && tmp[i]!='*' && tmp[i]!='(' && tmp[i] != ')' && tmp[i] != 's' && tmp[i] != 'i' && tmp[i] != 'n' && tmp[i] != 'c' && tmp[i] != 'o'){
    ...
    for(int i=0; i<tmp.size(); i++){
        ...
        if(tmp[i] == ')'){
            if(beg == -1){
                cout<<"Неверная последовательность скобок!"<<endl;
                return 0;
            }
            if(comp(tmp.substr(beg+1, i-beg)) == "Error"){
                return 0;
            }
